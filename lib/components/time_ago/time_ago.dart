...
String format(DateTime date,
  ...
  String result;
  if (seconds < 45) {
    result = LocaleKeys.now.tr();
  } else if (seconds < 90) {
    result ="${LocaleKeys.before.tr()} 1 ${LocaleKeys.minute_ago.tr()}"; /*messages.aboutAMinute(minutes.round())*/;
  } else if (minutes < 45) {
    result = "${LocaleKeys.before.tr()} ${minutes.round()} ${LocaleKeys.minute_ago.tr()}";/*messages.minutes(minutes.round());*/
  } else if (minutes < 90) {
    result = "${LocaleKeys.before.tr()} 1 ${LocaleKeys.hours_ago.tr()}"; /*messages.aboutAnHour(minutes.round());*/
  } else if (hours < 24) {
    result ="${LocaleKeys.before.tr()} ${hours.round()} ${LocaleKeys.hours_ago.tr()}"; /*messages.hours(hours.round());*/
  } /*else if (hours < 48) {
    result = "${hours.round()} ${LocaleKeys.days_ago.tr()}"; *//*messages.aDay(hours.round());*//*
  }*/ else if (days < 30) {
    result = "${LocaleKeys.before.tr()} ${days.round()} ${LocaleKeys.days_ago.tr()}";/*messages.days(days.round());*/
  } else if (days < 60) {
    result ="${LocaleKeys.before.tr()} ${months.round()} ${LocaleKeys.month_ago.tr()}"; /*messages.aboutAMonth(days.round());*/
  } else if (days < 365) {
    result ="${LocaleKeys.before.tr()} ${months.round()} ${LocaleKeys.month_ago.tr()}"; /*messages.months(months.round());*/
  } else if (years < 2) {
    result ="${LocaleKeys.before.tr()} ${years.round()} ${LocaleKeys.year_ago.tr()}";/* messages.aboutAYear(months.round());*/
  } else {
    result = "${LocaleKeys.before.tr()} ${messages.years(years.round())} ${LocaleKeys.year_ago.tr()}";
  }
